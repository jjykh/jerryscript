	...
	const jerry_length_t args_cnt) /**< number of function arguments */
{
		...
		&& jerry_value_is_string(args_p[0]))
	{
		...
		jerry_size_t str_size = jerry_get_string_size(args_p[0]);
		assert(str_size < 256);
		jerry_size_t sz = jerry_string_to_char_buffer(args_p[0], str_buf, str_size);
		assert(sz == str_size);
		str_buf[str_size] = 0;
		...
		if (source_p == NULL)
		{
			jerry_port_log(JERRY_LOG_LEVEL_ERROR, "Error loading file: %s\n", (char *) str_buf);
			exit(JERRY_STANDALONE_EXIT_CODE_FAIL);
		}

		jerry_value_t eval_ret = jerry_eval(source_p, source_size, false);
		return eval_ret;
	}
	jerry_port_log(JERRY_LOG_LEVEL_ERROR, "Error loading file\n");
	exit(JERRY_STANDALONE_EXIT_CODE_FAIL);
}
...
static void init_funcs(void) {
	...
	jerry_value_t load_func_name_val = jerry_create_string((jerry_char_t *) "load");
